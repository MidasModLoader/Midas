...
//=================================================================================================
/**
 * @brief Stack specialization for `std::wstring`.
 */
...
{
    [[nodiscard]] static Result push(lua_State* L, const std::wstring& str)
    {
#if LUABRIDGE_SAFE_STACK_CHECKS
        if (! lua_checkstack(L, 1))
            return makeErrorCode(ErrorCode::LuaStackOverflow);
    ...
    [[nodiscard]] static TypeResult<std::wstring> get(lua_State* L, int index)
    {
        std::size_t length = 0;
        const char* str = nullptr;
        ...
        std::wstring_convert<std::codecvt_utf8<wchar_t>> conv;
        return conv.from_bytes(str, str + length);
    }
